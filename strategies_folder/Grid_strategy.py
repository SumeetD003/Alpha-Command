...
API_URL = "https://api.geckoterminal.com/api/v2/networks/solana/pools/3bC2e2RxcfvF9oP22LvbaNsVwoS2T98q6ErCRoayQYdq/ohlcv/minute?aggregate=1&limit=1000 "
class GridTradingStrategy(Strategy):
    no_grids = 20
    capital_per_grid = 0.01  
    lookback = 75
    units_per_grid = 15 
    def init(self):
        self.grids = []
...
def fetch_ohlc_data(api_url):
    """Fetch OHLC data from the API and convert it to a pandas DataFrame."""
    try:
...
if __name__ == "__main__":
    ...
    # Fetch and preprocess SOLANA pool data
    ohlc_data = fetch_ohlc_data(API_URL)

    if len(ohlc_data) == 0:
        print("No data available for backtest. Exiting.")
    else:
        # Run the backtest with SOLANA data
        bt = Backtest(ohlc_data, GridTradingStrategy, cash=10000, commission=0.002, margin=0.02)
        stats = bt.run()
        bt.plot()
        print(stats)
        print(stats['_trades'])
