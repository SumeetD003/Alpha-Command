...
def html(
    ...
    download_filename="2cents-tearsheet.html",
    figfmt="svg",
    template_path=None,
    match_dates=True,
    trade_stats=None,  # New parameter for trade statistics
    trade_table=None,# New parameter for trade table (DataFrame)
    backtest=None,
    **kwargs,
):
    ...
    # Generate the Key Performance Metrics table (no filtering)
    mtrx = metrics(
        returns=returns,
        benchmark=benchmark,
        rf=rf,
    ...
    if trade_stats is not None:
        ...
        trade_stats_html = _html_table(trade_stats_df)
    ...
    if backtest is not None:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".html") as temp_file:
            ...
            backtest.plot(
                ...
                show_legend=True,
                open_browser=False,  # Don't open the browser
            )
    ...
    # Combine the Key Performance Metrics and Trade Statistics tables into a two-column layout
    combined_tables = f"""
    ...
    # Replace the metrics placeholder with the combined tables
    tpl = tpl.replace("{{metrics}}", combined_tables)

    # Replace the trade_details placeholder with the trade table
