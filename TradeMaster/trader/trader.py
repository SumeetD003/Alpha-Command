...
from TradeMaster.backtest import Backtest, Strategy
from TradeMaster.backtest.utils import calculate_positions
from TradeMaster.broker import Broker, BrokerAccount
from TradeMaster.datasource import QuoteSource
from TradeMaster.utils.config import config
from TradeMaster.utils.mailjet import mailjet_send_email
from TradeMaster.utils.mtdb import MTDB
from TradeMaster.utils.telegram import send_telegram_message
...
class TradePlan:
    ...
    __storage_root = os.path.expanduser('~/.Trademaster/storage')
...
class StrategyManager:
    ...
    __strategy_root = os.path.expanduser('~/.Trademaster/strategy')
...
class BacktestRunner:
    ...
    def execute(self, dryrun: bool = False) -> BacktestLog:
        ...
        try:
            ...
            proc = subprocess.run(
                [sys.executable, '-m', 'Trademaster.cli', 'backtest', '--run_id', self.run_id, self.plan.id] +
...
class TaskManager:
    ...
    __task_root = os.path.expanduser('~/.Trademaster/task')
