...
st.set_page_config(page_title='Trademaster Status', layout='wide')
...
def display_process_status():
    ...
    cmd = 'ps -o pid,ppid,command | grep python | grep Trademaster | grep -v grep'
...
def display_trademaster_version():
    st.caption('Version')
    st.text(version('Trademaster'))
...
st.header('Trademaster Status')

for func in [display_trademaster_version, display_process_status, display_trade_status, display_task_status,
