...
def __call_ibgateway_admin(method: str, path: str, params: dict | None = None):
    ...
    '''
    from TradeMaster.utils.config import config
...
def ib_start_inner(log_level=None):
    ...
    from TradeMaster.utils.config import config
    uvicorn.run(
        'Trademaster.broker.ibgateway:app',
...
def scheduler_start():
    ...
    from TradeMaster.utils.config import config
    uvicorn.run(
        'Trademaster.trader.scheduler:app',
...
def backtest(plan_id_or_name, run_id, dryrun, pytest):
    ...
    from TradeMaster.trader.trader import BacktestRunner
    if pytest:
        # import pytest which will trigger running in test mode using test db
        import pytest

    from TradeMaster.trader import TradePlan
    try:
...
def ib_diagnose():
    ...
    click.secho('Checking Trademaster installation:')
    items = [
        '~/.Trademaster',
        '~/.Trademaster/strategy',
        '~/.Trademaster/storage',
        '~/.Trademaster/ibgateway',
        '~/.Trademaster/config.yaml',
        '~/.Trademaster/database/Trademaster.db',
    ]
    for item in items:
...
def init(yes):
    '''Initialize Trademaster'''
    from importlib.metadata import version

    from TradeMaster.utils.config import trademaster_root

    check_prerequisites()

    if os.path.exists(trademaster_root) and not yes:
        click.secho(
            f'Trademaster is already installed in {trademaster_root}. '
            f'This will update the installation to version {version("Trademaster")}.\n'
    ...
    for d in ['database', 'strategy', 'storage', 'ibgateway', 'task']:
        os.makedirs(os.path.join(trademaster_root, d), mode=0o700, exist_ok=True)

    # upgrade config
    from TradeMaster.utils.config import GlobalConfig
    GlobalConfig.upgrade()

    # build db schema
    build_db_schema(trademaster_root)

    # download and extract IB gateway
    install_ibgateway(trademaster_root)

    # finish
    click.secho(f'Trademaster installed in {trademaster_root}', fg='green')


def build_db_schema(trademaster_root):
    click.secho(f'Setting up database...')
    db_loc = os.path.join(trademaster_root, 'database/Trademaster.db')
...
def install_ibgateway(trademaster_root):
    try:
        click.secho(f'Installing Interactive Brokers gateway...')
        ib_loc = os.path.join(trademaster_root, 'ibgateway')
        ...
        click.secho('We have some issue with installing IB gateway. If you use Trademaster solely for backtesting, you can safely ignore this.', fg='red')
