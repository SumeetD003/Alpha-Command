...
module directly, e.g.
    ...
    from synapse.backtest import Backtest, Strategy
"""
import functools
...
class Strategy(ABC):
    ...
    override methods
    `synapse.backtest.core.backtesting.Strategy.init` and
    `synapse.backtest.core.backtesting.Strategy.next` to define
    your own strategy.
    """
    ...
    def I(self,  # noqa: E743
        ...
        `synapse.backtest.core.backtesting.Strategy.next` much like
        `synapse.backtest.core.backtesting.Strategy.data` is.
        ...
        same length as `synapse.backtest.core.backtesting.Strategy.data`, or
        ...
        `synapse.backtest.core.backtesting.Backtest.plot`.
    ...
    def init(self):
        ...
        Declare indicators (with `synapse.backtest.core.backtesting.Strategy.I`).
        Precompute what needs to be precomputed or can be precomputed
        in a vectorized fashion before the strategy starts.

        If you extend composable strategies from `synapse.backtest.core.backtesting.lib`,
    ...
    def next(self):
        ...
        `synapse.backtest.core.backtesting.Strategy.data`
        instance (row; full candlestick bar) becomes available.
        This is the main method where strategy decisions
        upon data precomputed in `synapse.backtest.core.backtesting.Strategy.init`
        take place.
    ...
    def record(self, name: str = None, plot: bool = True, overlay: bool = None, color: str = None, scatter: bool = False, **kwargs):
        ...
        Args:
            ...
            plot: If True, the time series will be plotted on the resulting `synapse.backtest.core.backtesting.Backtest.plot`.
    ...
    def data(self) -> _Data:
        ...
        `synapse.backtest.core.backtesting.Backtest.__init__`,
        ...
        * Within `synapse.backtest.core.backtesting.Strategy.init`, `data` arrays
          are available in full length, as passed into
          `synapse.backtest.core.backtesting.Backtest.__init__`
          (for precomputing indicators and such). However, within
          `synapse.backtest.core.backtesting.Strategy.next`, `data` arrays are
          only as long as the current iteration, simulating gradual
          price point revelation. In each call of
          `synapse.backtest.core.backtesting.Strategy.next` (iteratively called by
          `synapse.backtest.core.backtesting.Backtest` internally),
    ...
    def position(self, ticker: str = None) -> 'Position':
        """Instance of `synapse.backtest.core.backtesting.Position`.
...
class Position:
    ...
    `synapse.backtest.core.backtesting.Strategy.position` within
    `synapse.backtest.core.backtesting.Strategy.next`.
    ...
    def next(self):
        ...
        if equity[0] <= 0:
            if not self._is_option: 
                ...
                warnings.warn("Equity is below zero, but option trades are still open.", UserWarning)
...
class Backtest:
    ...
    `synapse.backtest.core.backtesting.Backtest.run` to run a backtest
    instance, or `synapse.backtest.core.backtesting.Backtest.optimize` to
    optimize it.
    """
    ...
    def __init__(self,
        ...
        `strategy` is a `synapse.backtest.core.backtesting.Strategy`
        ...
        # Check if the class inherits from Strategy at any level in its inheritance chain
        if not (isinstance(strategy, type) and any(base.__name__ == 'Strategy' for base in strategy.__mro__)):
    ...
    def optimize(self, *,
        ...
        `synapse.backtest.core.backtesting.Backtest.run`-returned results series,
        ...
        * `"skopt"` which finds close-to-optimal strategy parameters using
        ...
        def _optimize_grid() -> Union[pd.Series, Tuple[pd.Series, pd.Series]]:
                ...
                if mp.get_start_method(allow_none=False) == 'fork':
                    with ProcessPoolExecutor() as executor:
                        ...
                        for future in _tqdm(as_completed(futures), total=len(futures),
                                            desc='Backtest.optimize', disable = True):
                    ...
                    for batch_index in _tqdm(range(len(param_batches)), disable = True):
            ...
            if heatmap.isna().all():
                ...
                stats = self.run(**param_combos[0])  # Fallback to a default set of parameters or handle accordingly
            else:
                best_params = heatmap.idxmax(skipna=True)
                stats = self.run(**dict(zip(heatmap.index.names, best_params)))
        ...
        def _optimize_skopt() -> Union[pd.Series,
            ...
            INVALID = 1e300
            progress = iter(_tqdm(repeat(None), total=max_tries, desc='Backtest.optimize', disable = True))
    ...
    def plot(self, *, results: pd.Series = None, filename=None, plot_width=None,
        ...
        `synapse.backtest.core.backtesting.Backtest.run` or
        `synapse.backtest.core.backtesting.Backtest.optimize`, otherwise the last
    ...
    def tear_sheet(self, *, results: pd.Series = None, filename=None, open_browser=True, output_path=None):
        """
        ...
        - open_browser: bool, whether to open the tearsheet in the default browser. Default is True.
        - output_path: str, the directory path to save the HTML report. If None, uses the current directory.
        """
        if results is None:
            if self._results is None:
                raise RuntimeError('First issue `backtest.run()` to obtain results.')
        ...
        # Extract the 'Close' column for the benchmark
        close_column = [col for col in self._data.columns if col[1] == 'Close']
        close_column = close_column[0]
        
        # Prepare equity series (strategy returns)
        equity_df = results['_equity_curve']
        equity_series = equity_df['Equity'].pct_change().dropna()
        
        # Prepare benchmark series
        benchmark_series = self._data[close_column].pct_change().dropna()
        benchmark_series = benchmark_series.rename("Benchmark")

        # Prepare trade statistics and trade details
        trade_stats = results.drop(['_equity_curve', '_trades', '_orders', '_positions'])  # Exclude non-statistic fields
        trade_table = results['_trades']  # The trade details DataFrame

        # Determine the output path for the tearsheet
        ...
        # Generate the tearsheet with trade stats and trade table
        quantstats.reports.html(
            equity_series,
            benchmark=benchmark_series,
            output=tear_sheet_path,
            trade_stats=trade_stats,  # Pass the trade statistics
            trade_table=trade_table,  # Pass the trade details table
        )
